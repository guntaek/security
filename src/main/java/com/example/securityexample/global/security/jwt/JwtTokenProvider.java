...
public class JwtTokenProvider {
    ...
    public JwtTokenProvider(@Value("${jwt.secret}") String secretKey,
                            @Value("${jwt.access-token-expire-seconds}") long accessTokenExpireSeconds, JwtTokenRepository jwtTokenRepository) {
        this.accessTokenExpireSeconds = accessTokenExpireSeconds;
        byte[] keyBytes = Decoders.BASE64.decode(secretKey);
        this.key = Keys.hmacShaKeyFor(keyBytes);

        this.jwtTokenRepository = jwtTokenRepository;  //내가 한거
    }
    ...
    public JwtTokenDto createToken(Authentication authentication) {
        ...
        String authorities = authentication.getAuthorities().stream()
                ...
                .collect(Collectors.joining(",")); //권한이 1개 이상일때 권한을 가져오는 코드
        ...
        jwtTokenRepository.save(JwtToken.builder()
                .email(memberUserDetails.getMember().getEmail())
